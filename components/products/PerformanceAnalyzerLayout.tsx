...
export default function PerformanceAnalyzerLayout({ product }: { product: ProductData }) {
  return (
    <div className="min-h-screen pt-24 pb-16 lg:pt-32 lg:pb-24 bg-white text-black px-4 md:px-8 flex justify-center">
      {/* Background Grid */}
      ...
      <div className="w-full max-w-[85rem] border-[4px] border-black bg-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] flex flex-col overflow-hidden relative z-10">
        ...
        <div className="w-full border-b-[4px] border-black bg-white p-6 md:p-10 flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6">
          <div className="max-w-3xl">
            <h1 className="text-3xl md:text-5xl font-black uppercase leading-none tracking-tighter mb-4 break-all sm:break-normal text-black border-b-[4px] border-black pb-2 inline-block">
              {product.h1}
            </h1>
            <br />
            <p className="text-sm md:text-base font-black text-gray-800 uppercase mt-4">
              {product.subheadline}
            </p>
          </div>
        ...
        <div className="w-full border-b-[4px] border-black bg-gray-100 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="col-span-1 border-b-[4px] lg:border-b-0 lg:border-r-[4px] border-black pb-6 lg:pb-0 lg:pr-6 flex flex-col justify-center">
            <h2 className="text-base md:text-lg font-black text-black uppercase tracking-widest border-b-[4px] border-black pb-2 self-start">Overview</h2>
          </div>
        ...
        <div className="w-full flex flex-col lg:flex-row border-b-[4px] border-black bg-white">
          ...
          <div className="w-full lg:w-1/2 flex flex-col bg-gray-50">
            <div className="bg-blue-600 text-white p-4 border-b-[4px] border-black uppercase text-xs md:text-sm font-black tracking-widest">
              Sample Finding
            </div>
              ...
              {product.exampleFinding && (
                <div className="mb-8">
                  <div className="border-[4px] border-black p-5 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                    <div className="flex flex-col sm:flex-row justify-between items-start mb-5 gap-3 border-b-[4px] border-black pb-4">
                      <h3 className="font-black text-sm md:text-base uppercase text-black">{product.exampleFinding.title}</h3>
                    ...
                    <div className="bg-green-100 border-[4px] border-black p-4">
                      <span className="text-[10px] font-black text-green-800 block mb-2 uppercase tracking-widest border-b-[2px] border-green-800 pb-1">Recommended Fix Protocol</span>
                      <pre className="text-[10px] md:text-xs font-mono font-bold text-black border-l-[4px] border-green-800 pl-2 whitespace-pre-wrap">{product.exampleFinding.fix}</pre>
                    </div>
                  </div>
                </div>
              )}
