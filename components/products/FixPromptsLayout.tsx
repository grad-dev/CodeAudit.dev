    ...
    <div className="min-h-screen pt-24 pb-16 lg:pt-32 lg:pb-24 bg-white font-mono text-black px-4 md:px-8">
      ...
      <div className="w-full max-w-[85rem] mx-auto border-[4px] border-black bg-white flex flex-col shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] relative z-10">
        ...
        <div className="border-b-[4px] border-black bg-blue-600 text-white p-4 md:p-6 flex justify-between items-center">
          <h1 className="text-2xl md:text-4xl font-black uppercase tracking-tighter leading-[1]">{product.h1} {"// TERMINAL"}</h1>
          <span className="bg-yellow-300 text-black px-3 py-1 text-[10px] md:text-xs font-black border-[4px] border-black shadow-[2px_2px_0_0_#000]">STATUS: ONLINE</span>
        </div>
          ...
          <div className="flex flex-col items-end w-full">
            <div className="w-full md:w-[80%] border-[4px] border-black bg-white p-4 md:p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
              <div className="flex justify-between border-b-[4px] border-black pb-2 mb-4">
                <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-blue-600">USER_PROMPT_INIT</span>
                <span className="text-[10px] md:text-xs font-black uppercase">{new Date().toISOString().split('T')[1].split('.')[0]}</span>
              </div>
              <p className="text-xs md:text-sm font-black uppercase leading-snug">{product.subheadline}</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-start w-full">
            <div className="w-full md:w-[90%] border-[4px] border-black bg-white p-4 md:p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
              <div className="flex justify-between border-b-[4px] border-black pb-2 mb-4 bg-black text-white p-2">
                <span className="text-[10px] md:text-xs font-black uppercase tracking-widest text-green-400">SYSTEM_RESPONSE // AGENT</span>
                    ...
                    {product.promptCards.map((card, idx) => (
                      <div key={idx} className="border-[4px] border-black bg-white p-4 relative shadow-[4px_4px_0_0_#000]">
                        ...
                        <div className="mb-4 bg-gray-50 p-2 border-[2px] border-black">
                          <span className="font-black block mb-2 text-[10px] md:text-xs bg-red-500 text-white inline-block px-2 py-0.5">ISSUE_DETECTED:</span>
                          <p className="text-[10px] md:text-xs font-bold border-l-[4px] border-red-500 pl-3 py-1">{card.issue}</p>
                        </div>
                        <div>
                          <span className="font-black block mb-2 text-[10px] md:text-xs bg-black text-white inline-block px-2 py-0.5">RECOMMENDED_FIX_PROMPT:</span>
              ...
              <div className="mb-6">
                <span className="bg-black text-white px-3 py-1 text-[10px] md:text-xs font-black uppercase mb-3 inline-block tracking-widest">WORKFLOW_INSTRUCTION</span>
                <p className="text-[10px] md:text-xs border-[4px] border-black p-4 bg-yellow-300 font-bold shadow-[4px_4px_0_0_#000]">
